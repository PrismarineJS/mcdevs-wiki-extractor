...
function extractProtocol(fnPacket, sectionObject,cb)
{
  ...
  async.map(notStates(sectionObject), function(state, cb) {
      async.map(notStates(state), function(direction, cb) {
          async.map(notStates(direction), function(packet, cb) {
...
function commentsForPacket(packetText, cb) {
  ...
  pandoc(packet.before.join("\n"), "mediawiki", "markdown_github", function(err, result) {
