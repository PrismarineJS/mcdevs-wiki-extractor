...
function writeProtocol(protocolFilePath)
{
  async.waterfall([
      getProtocol
    ],
    function(err,protocol) {
      write(err,protocol,protocolFilePath);
    }
  );
}
...
function write(err,protocol,protocolFilePath){
  if(err)
  {
  ...
  fs.writeFile(protocolFilePath, JSON.stringify(protocol,null,2));
}
